...
def convert_interactions_to_events(val: Any) -> List[types.evals.Event]:
    ...
    for item in interactions:
        ...
        fr_part = genai_types.Part.from_function_response(name=tool_name, response=response)
...
def map_dataset_columns(
    ...
    for placeholder, details in mapping.items():
        if "source_column" in details:
            ...
            cands = [col_path.replace(":", "."), f"extracted_data.{col_path}", f"reference_data.{col_path}", col_path]
            ...
            if source_col:
                ...
                if root_key in original_df.columns:
                    val_series = original_df[root_key].apply(lambda x: get_nested_value(robust_json_loads(x), col_path))
            ...
            if val_series is not None:
                ...
                if is_tool_metric and is_event_col:
                    eval_dataset[placeholder] = val_series.apply(convert_interactions_to_events)
                else:
            ...
            def format_template(row):
                ...
                for sc in source_cols:
                    cands = [sc.replace(":", "."), f"extracted_data.{sc}", f"reference_data.{sc}", sc]
                    found_sc = next((c for c in cands if c in row.index and row[c] is not None), None)
                    template_vars[sc.replace(":", "_")] = row[found_sc] if found_sc else ""
                return details["template"].format(**template_vars) if template_vars else details["template"]
