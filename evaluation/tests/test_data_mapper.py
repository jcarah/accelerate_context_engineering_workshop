...
from evaluation.utils.data_mapper import (
    map_dataset_columns,
    get_nested_value,
    robust_json_loads,
)
...
class TestDataMapper:
    ...
    def sample_df(self):
        ...
        data = {
            ...
            "extracted_data": [
                json.dumps({"state_variables": {"profile": {"name": "Alex"}}})
            ],
    ...
    def test_tool_use_conversion_managed(self, sample_df):
        ...
        result = map_dataset_columns(
            ...
            mapping,
            "TOOL_USE_QUALITY",
            metric_tool_use_name="TOOL_USE_QUALITY",
        )
    ...
    def test_tool_use_flattening_custom(self, sample_df):
        ...
        mapping = {"tool_usage": {"source_column": "tool_interactions"}}
        # metric_name is NOT TOOL_USE_QUALITY, so it should flatten
        result = map_dataset_columns(
            sample_df, sample_df, mapping, "custom_tool_metric"
        )
