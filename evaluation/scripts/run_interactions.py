...
def parse_metadata_filters(filter_strings):
    ...
    for filter_string in filter_strings:
        if ":" not in filter_string:
            print(
                f"Warning: Invalid filter format '{filter_string}'. Expected 'key:value1,value2'"
            )
            continue
...
async def process_question(question_data, agent_client, run_id, args, state_vars):
    ...
    try:
        ...
        print(
            f"--- Completed interaction for question '{user_inputs}' (Run {run_id}) SUCCESSFULLY ---"
        )
        return result_dict
    except Exception as e:
        error_message = f"An unexpected error occurred: {e}"
        print(
            f"--- ERROR running interaction for question '{user_inputs}' (Run {run_id}) ---"
        )
        print(error_message)
        return {
...
async def main():
    parser = argparse.ArgumentParser(
        description="Run agent interactions for evaluation."
    )
    parser.add_argument(
        "--user_id", type=str, default="e2e_test_user", help="The user ID to use."
    )
    parser.add_argument(
        "--base_url",
        type=str,
    ...
    parser.add_argument(
        "--app_name", type=str, required=True, help="The name of the application/agent."
    )
    parser.add_argument(
        "--questions_file",
        type=str,
        default="tests/datasets/su_golden_questions.json",
    ...
    parser.add_argument(
        ...
        help="Directory to save the interaction results.",
    )
    parser.add_argument(
        "--user",
        type=str,
        default=os.environ.get("USER"),
        help="The LDAP of the user running the script.",
    )
    parser.add_argument(
        "--filter",
        action="append",
        dest="metadata_filters",
        help="Filter questions by metadata.",
    )
    parser.add_argument(
        "--state-variable",
        action="append",
        dest="state_variables",
        help="Inject state variables during session creation (e.g., 'key:value').",
    )
    parser.add_argument(
        "--runs",
        type=int,
    ...
    if not args.user:
        parser.error(
            "The --user argument is required or the USER environment variable must be set."
        )
    ...
    if args.metadata_filters:
        ...
        local_golden_questions = filter_questions_by_metadata(
            local_golden_questions, filters
        )
    ...
    print(
        f"Running interactions for {len(questions_to_run)} questions ({args.runs} times each)"
    )

    agent_client = AgentClient(
        base_url=args.base_url, app_name=args.app_name, user_id=args.user_id
    )
    ...
    if all_results:
        ...
        output_csv_path = args.output_csv or os.path.join(
            args.results_dir,
            f"interaction_{os.path.basename(args.questions_file).replace('.json', '')}.csv",
        )
        df.to_csv(output_csv_path, index=False)
        print(
            f"Interaction results for {len(all_results)} runs saved to {output_csv_path}"
        )

    failed_questions = [
        res
        for res in task_results
        if json.loads(res.get("status")).get("boolean") == "failed"
    ]
    if failed_questions:
