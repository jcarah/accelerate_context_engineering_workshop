...
def convert_test_to_golden(
    input_path, output_path, agent_name, metadata_pairs, id_prefix="q"
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert turn-based test data to golden evaluation dataset format."
    )
    parser.add_argument(
        "--input", required=True, help="Path to the input JSON file (list of turns)."
    )
    parser.add_argument(
        "--output", required=True, help="Path to the output golden dataset JSON file."
    )
    parser.add_argument(
        "--agent", default="customer_service", help="Name of the agent being evaluated."
    )
    parser.add_argument(
        "--metadata",
        action="append",
        dest="metadata",
        help="Metadata labels (e.g., 'complexity:easy', 'tenant:cymbal'). Can be used multiple times.",
    )
    parser.add_argument("--prefix", default="q", help="Prefix for the question ID.")

    args = parser.parse_args()

    convert_test_to_golden(
        args.input, args.output, args.agent, args.metadata, args.prefix
    )
