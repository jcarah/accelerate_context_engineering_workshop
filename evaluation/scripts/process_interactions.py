...
async def process_interaction(row, results_dir=None, skip_traces=False):
    ...
    try:
        ...
        if status_.get("boolean") == "failed":
            raise ValueError(
                "Interaction failed in the previous step, run the script again or remove the failed rows in the interaction CSV."
            )
        ...
        if skip_traces:
            print(
                f"Skipping trace retrieval for session {session_id} (--skip-traces flag enabled)"
            )
            session_trace = None
        else:
            try:
        ...
        if results_dir:
            ...
            output_file_json = os.path.join(
                results_dir, f"session_{question_filename}_{session_id}.json"
            )
            output_file_trace_json = os.path.join(
                results_dir, f"trace_{question_filename}_{session_id}.json"
            )
        ...
        if not session_trace:
            print(
                f"Warning: Failed to retrieve session trace for session {session_id}. Proceeding without trace data."
            )
            ...
            row["missing_information"] = json.dumps(
                {
                    "boolean": True,
                    "details": "Missing session trace, hence, also latency_data, trace_summary.",
                }
            )
        else:
        ...
        extracted_data["sub_agent_trace"] = AgentClient.get_sub_agent_trace(
            final_session_state
        )
        ...
        row["missing_information"] = json.dumps(
            {
                "boolean": True,
                "details": f"Error processing session and/or trace. Error: {e}",
            }
        )
        return row
...
async def main():
    parser = argparse.ArgumentParser(
        description="Process agent interactions to extract trace and state data."
    )
    parser.add_argument(
        "--input-csv",
        required=True,
        help="Path to the input CSV file with interaction results.",
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        "--output-csv", help="Exact path to the output CSV file with enriched data."
    )
    parser.add_argument(
        "--skip-traces",
        action="store_true",
        help="Skip trace retrieval entirely (recommended for ADK 1.15 when trace endpoints are unavailable).",
    )
    ...
    if args.skip_traces:
        ...
        print(
            "Evaluation will proceed using session state only. Latency metrics will not be available."
        )

    tasks = [
        process_interaction(row, args.results_dir, args.skip_traces)
    ...
    missing_data_df["boolean"] = missing_data_df["missing_information"].apply(
        lambda x: json.loads(x).get("boolean")
    )
    missing_data_df = missing_data_df[missing_data_df["boolean"]]
    ...
    if trace_missing_count > 0:
        ...
        print(
            "Session IDs:",
            ", ".join(trace_missing_df["session_id"].astype(str).tolist()),
        )
    ...
    if other_missing_count > 0:
        ...
        print(
            "Session IDs:",
            ", ".join(other_missing_df["session_id"].astype(str).tolist()),
        )

    output_csv = args.output_csv or os.path.join(
        args.results_dir, f"processed_{os.path.basename(args.input_csv)}"
    )
